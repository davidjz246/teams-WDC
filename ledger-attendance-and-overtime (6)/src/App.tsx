...
export default function App() {
  ...
  const [exportSettings, setExportSettings] = useState<ExportSettings>(() => {
    ...
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved);
        return {
          name: nameParam ? decodeURIComponent(nameParam) : parsed.name || 'David Joseph Zakria Ibrahim Kalad',
          employeeId: idParam || parsed.employeeId || '32272',
    ...
    return {
      name: nameParam ? decodeURIComponent(nameParam) : 'David Joseph Zakria Ibrahim Kalad',
      employeeId: idParam || '32272',
      shiftEnd: '17:00',
    };
  });
  ...
  const [dayReasons, setDayReasons] = useState<Record<string, string>>(() => {
    ...
    return {
      '2026.07.27': 'Release deployment support & system testing',
      '2026.07.28': 'Month-end ledger balancing and audit review',
      '2026.08.03': 'Urgent migration patch and post-deployment validation',
      '2026.08.05': 'Attendance sprint closing',
    };
  });
  ...
  // Load Demonstration sample data
  const handleLoadSampleDemo = () => {
    setRawInput(DEFAULT_RAW_PUNCH);
    setOverrides({});
    setPermissionsFiled({});
    setAbsenceCheckpoints({});
    setWeekendDays([5, 6]);
    setExportSettings({
      name: 'David Joseph Zakria Ibrahim Kalad',
      employeeId: '32272',
      shiftEnd: '17:00',
    });
    setDayReasons({
